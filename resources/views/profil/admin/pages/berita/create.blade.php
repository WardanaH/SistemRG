{{-- resources/views/profil/admin/pages/berita/create.blade.php --}}
...
<style>
  ...
  .rg-edbtn:active{ transform: translateY(0px); }
  .rg-edbtn.is-active{
    background:#0f172a;
    color:#fff;
    border-color:#0f172a;
  ...
  /* Word-like: font size via data-fs */
  ...
  /* paragraph spacing after via data-pa */
  ...
  .rg-editor__hint b{ font-weight: 950; }
</style>
...
<div class="rg-page-wrap">
  ...
  <form id="newsForm" action="{{ route('profil.admin.berita.store') }}" method="POST" enctype="multipart/form-data">
    @csrf

    <div class="rg-topbar">
      <div>
        <div class="t">Tambah Berita</div>
        <div class="s">
          Konten ini akan muncul di halaman <b>Berita</b> dan juga bisa dipakai untuk <b>3 berita terbaru</b> di Beranda (tipe: <b>news</b>).
        </div>
      </div>
    ...
    <div class="rg-card-shell">
      <div class="rg-card-head">
        ...
        <div class="text-muted small">Isi data lalu klik <b>Simpan</b>.</div>
      </div>
        ...
        <div class="row g-3">
          <div class="col-12 col-lg-8">
            <div class="rg-section-card">
              ...
              <select class="form-select" name="type">
                <option value="news" @selected($ov('type','news')==='news')>Berita</option>
                <option value="education" @selected($ov('type','news')==='education')>Edukasi</option>
              </select>

              <div class="row g-3 mt-1">
                <div class="col-12">
                  <label class="form-label fw-bold">Judul</label>
                  <input class="form-control" name="title" value="{{ $ov('title') }}" placeholder="Mis: Project Neonbox Baru">
                </div>

                <div class="col-12 col-md-6">
                  <label class="form-label fw-bold">Slug (opsional)</label>
                  <input class="form-control" name="slug" value="{{ $ov('slug') }}" placeholder="otomatis dari judul jika kosong">
                  <div class="rg-help mt-1">Kalau kosong, slug otomatis dibuat dari judul.</div>
                </div>

                <div class="col-12 col-md-6">
                  <label class="form-label fw-bold">Kategori Label (opsional, untuk Edukasi)</label>
                  <input class="form-control" name="category_label" value="{{ $ov('category_label') }}" placeholder="Mis: Printing / Desain">
                </div>

                <div class="col-12">
                  <label class="form-label fw-bold">Ringkasan (Excerpt)</label>
                  <textarea class="form-control" name="excerpt" rows="3" placeholder="Ringkasan untuk card/list">{{ $ov('excerpt') }}</textarea>
                </div>

                <div class="col-12">
                  <label class="form-label fw-bold">Isi Konten</label>

                  {{-- hidden real field (yang submit ke server) --}}
                  <textarea id="contentHidden" class="d-none" name="content">{{ $ov('content') }}</textarea>
                  ...
                  <div class="rg-content-actions">
                    ...
                    <div class="rg-help">
                      Konten akan dibuka di popup editor. Hasil disimpan sebagai HTML yang clean.
                    </div>
                  </div>
          ...
          <div class="col-12 col-lg-4">
            <div class="rg-section-card">
              ...
              <input type="datetime-local" class="form-control" name="published_at" value="{{ $ov('published_at') }}">
              <div class="rg-help mt-1">Kalau kosong, otomatis pakai waktu sekarang.</div>

              <div class="form-check mt-3">
                <input class="form-check-input" type="checkbox" value="1" id="isPublished" name="is_published" @checked(old('is_published', true))>
              ...
              <div class="rg-help mt-2">Jika tidak diupload, public akan tampil placeholder.</div>
            </div>
          </div>
        </div>
      </div>
    ...
    {{-- ✅ SKIP MODAL CONFIRM SAVE GLOBAL (biar submit langsung, optional) --}}
    {{-- <input type="hidden" name="_skip_confirm" value="1"> --}}
  </form>
</div>

{{-- ============================
   MODAL: CONTENT EDITOR (BOOTSTRAP 5)
   ============================ --}}
    ...
    <div class="modal-content">
      <div class="modal-header">
        <div>
          ...
          <div class="text-muted small" style="line-height:1.6;">Gunakan toolbar seperti Word (tanpa heading/link/image).</div>
        </div>
        ...
        <div class="rg-editor" data-rg-editor>
          ...
          <div class="rg-editor__body">
            ...
            <div class="rg-editor__hint">
              <div>💡</div>
              <div>
                Bisa <b>paste</b> dari Word/Google Docs. Sistem akan rapikan HTML supaya hasil clean.
              </div>
            </div>
          </div>
        </div>
...
(function(){
  ...
  // ==============
  // SANITIZER (NO DEP) - allowlist tags + safe attrs
  // ==============
  ...
  function sanitizeHtml(html){
    ...
    const walk = (node) => {
      if(node.nodeType === Node.ELEMENT_NODE){
        ...
        // remove forbidden tags by unwrapping content
        ...
        attrs.forEach(a => {
          ...
          // kill event handlers/styles/classes/ids
          ...
          // allow data-fs only on SPAN (font size)
          ...
          // allow data-pa only on block-ish tags (paragraph spacing after)
          ...
          // remove any other attrs
          node.removeAttribute(a.name);
        });

        // unwrap empty span (no data-fs)
        if(tag === 'SPAN'){
  ...
  // when modal opens: load hidden value
  ...
  // save modal -> sanitize -> store to hidden + preview
  ...
  // on submit: ensure hidden sanitized
